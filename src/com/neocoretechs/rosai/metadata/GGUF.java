...
public final class GGUF {
    ...
    void loadModelImpl(FileChannel fileChannel) throws IOException {
    	loadModelImpl(fileChannel,false);
    }
    
    void loadModelImpl(FileChannel fileChannel, boolean loadTensors) throws IOException {
    ...
    public static Map<String, GGMLTensorEntry> loadTensors(FileChannel fileChannel, long tensorDataOffset, Map<String, GGUFTensorInfo> tensorInfos) throws IOException {
        MemorySegment tensorData = fileChannel.map(FileChannel.MapMode.READ_ONLY, tensorDataOffset, fileChannel.size() - tensorDataOffset, Llama3.sharedArena);
        Map<String, GGMLTensorEntry> tensorEntries = HashMap.newHashMap(tensorInfos.size());
        if(DISPLAY_METADATA)
        	System.out.println("Begin Tensors:");
        ...
        if(DISPLAY_METADATA)
        	System.out.println("End Tensors.\r\n");
